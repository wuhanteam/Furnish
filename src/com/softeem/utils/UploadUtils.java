...
public class UploadUtils {
	...
	// 文件保存目录相对路径
	private String basePath = Constants.UPLOAD_BASE_PATH;
	// 文件的目录名
	...
	public List<HashMap<String,String>> uploadFile(HttpServletRequest request) {
		String[] infos = new String[1];
		List<HashMap<String,String>> fileUploadInfolist = new ArrayList<HashMap<String,String>>();
		// 验证
		...
		if (fiList != null) {
			...
			for (FileItem item : fiList) {
				...
				String error  = this.saveFile(item);
...
//	@SuppressWarnings("unchecked")
	...
	private String saveFile(FileItem item) {
		...
		if (item.getSize() > maxSize) { // 检查文件大小
		    ...
		    //E:/yuanhao/upload/images/20170305/
			fileUrl = newFileName;
			if(savePath.contains(Constants.UPLOAD_BASE_PATH)){
				fileUrl = savePath.substring(Constants.UPLOAD_BASE_PATH.length())+newFileName;
			}
			try {
