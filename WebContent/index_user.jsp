...
<html>
	...
	function atagClick(tag){
		
		if(tag==5){//退出系统
			var url ="${ctx}/logout.action";
			window.location.href = url;
		}else{
			var url ="${ctx}/menuServlet.action?flag="+tag;
			window.frames['right'].location.href = url;
		}
	}
	...
	<body>
	    ...
	    <div id="container_left">
	        <div id="left_top">
	            ...
	            <div class="left_welcome">${loginUser}欢迎登入!</div>
	        </div>
	    
	        <!-- 左侧菜单 -->
	        <div id="menu">
	            <ul>
	                <li class="menuli" class="menuli"  onclick="atagClick(1)">用户管理</li>
	                <li class="menuli"  onclick="atagClick(2)">客户管理</li>
	                <li class="menuli"  onclick="atagClick(3)">商品信息录入</li>
	                <li class="menuli"  onclick="atagClick(4)">1元夺宝</li>
	                <li class="menuli"  onclick="atagClick(5)">退出系统</li>
	            </ul>
	        </div>
		</div>
		<iframe name="right" src="${ctx}/right.jsp" width="80%" height="100%" style="float:right" frameborder="no" border="0" marginwidth="0" marginheight="0"></iframe>
   </div>
     <!--弹出层确认删除-->
